  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, "students"), {
        ...
        email: `${formData.name.toLowerCase().replace(/\s+/g, '')}@trial.com`,
        planStatus: 'Active',
        plan: 'Free Trial',
        joinDate: new Date().toISOString().split('T')[0],
        createdAt: serverTimestamp(),
        source: 'Hero Lead Form SEO'
      });
        ...
        <div className="flex flex-col items-center text-center animate-fade-up">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/95 backdrop-blur-md rounded-full border border-brand-primary/10 shadow-sm mb-8">
            ...
            <span className="text-xs font-bold text-brand-dark/70 uppercase tracking-wider">
              <span className="font-black text-brand-primary">{liveCount}</span> Students Practicing #1 CA Test Series Online
            </span>
          </div>
          ...
          <p className="text-lg md:text-xl text-brand-dark/60 font-medium leading-relaxed mb-12 max-w-2xl">
            Boost your <strong>CA Exam preparation</strong> with India's most trusted test series. Get your answer sheets evaluated by <strong className="text-brand-dark font-bold underline decoration-brand-orange decoration-2 underline-offset-4">AIR Rankers</strong> within 48 hours. Designed for the <strong>ICAI New Scheme 2024</strong>.
          </p>
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl shadow-brand-primary/10 border border-white p-2">
              <div className="bg-slate-50/50 rounded-[2rem] p-6 md:p-10">
                <div className="flex flex-col items-center gap-2 mb-8">
                  ...
                  <p className="text-[10px] font-black text-brand-dark/40 uppercase tracking-[0.2em]">AIR Ranker Evaluation & Feedback</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                  <div className="relative group">
                    ...
                    <input 
                      type="text" placeholder="Full Name" aria-label="Aspirant Name" required
                  ...
                  <div className="relative group">
                    ...
                    <input 
                      type="tel" placeholder="WhatsApp No." aria-label="WhatsApp Number for OTP" required maxLength={10}
                      className="w-full pl-12 pr-4 py-4 bg-white border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-brand-primary/5 outline-none transition-all"
                  ...
                  <div className="relative sm:col-span-2">
                    <select 
                      ...
                      value={formData.course} aria-label="Select CA Exam Course" onChange={(e) => setFormData({...formData, course: e.target.value})}
                    >
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="flex -space-x-3">
              {[1,2,3,4,5].map(i => (
                <div key={i} className="w-12 h-12 rounded-full border-4 border-brand-cream bg-slate-200 shadow-sm overflow-hidden">
                  <img src={`https://i.pravatar.cc/100?img=${i+60}`} alt="Successful CA Ranker Student" className="w-full h-full object-cover" loading="lazy" />
                </div>
              ))}
            </div>
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm font-bold text-brand-dark/60"><span className="text-brand-dark font-black">4.9/5 Rating</span> from 5,000+ Verified CA Students</p>
            </div>
          </div>
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-8 border-t border-brand-primary/10 pt-10">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-[10px] font-black uppercase text-brand-dark/40 tracking-widest">CA Success Rate</p>
                <p className="text-sm font-black text-brand-dark">87% Ranker Ratio</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-sm font-black text-brand-dark">100% AIR Feedback</p>
              </div>
            </div>
          </div>
